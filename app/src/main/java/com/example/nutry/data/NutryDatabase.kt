...
abstract class NutryDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Remove lastEaten column from ingredients table
                database.execSQL("CREATE TABLE ingredients_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name TEXT NOT NULL, categoryId INTEGER NOT NULL, emoji TEXT NOT NULL DEFAULT '', FOREIGN KEY(categoryId) REFERENCES categories(id) ON DELETE CASCADE)")
                database.execSQL("INSERT INTO ingredients_new (id, name, categoryId, emoji) SELECT id, name, categoryId, emoji FROM ingredients")
                database.execSQL("DROP TABLE ingredients")
                database.execSQL("ALTER TABLE ingredients_new RENAME TO ingredients")
                database.execSQL("CREATE INDEX IF NOT EXISTS index_ingredients_categoryId ON ingredients (categoryId)")
            }
        }

        private val MIGRATION_7_8 = object : Migration(7, 8) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Add isAutoGenerated column to track_entries table
                database.execSQL("ALTER TABLE track_entries ADD COLUMN isAutoGenerated INTEGER NOT NULL DEFAULT 0")
            }
        }

        private val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Add parentTrackId column to track_entries table
                database.execSQL("ALTER TABLE track_entries ADD COLUMN parentTrackId INTEGER")
            }
        }
